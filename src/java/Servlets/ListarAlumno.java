        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            for (Alumno p : ac.findAlumnoEntities()) {
                ...
                if (p.getEstado().equals("a")) {
                    j.addProperty("estado", "<button class='btn btn-success'>Activo</button>");
                }else if (p.getEstado().equals("n")) {
                    j.addProperty("estado", "<button class='btn btn-danger'>No activo</button>");
                }else{
                    j.addProperty("estado", "<button class='btn btn-danger'> -- </button>");
                ...
                j.addProperty("apoderados", "<a  class='btn' href='apoderados.jsp?alumno="+p.getId()+"&nombre="+p.getNombre() + " " + p.getApPaterno() + " " + p.getApMaterno()+"' \"><i class='fa fa-users text-inverse m-r-10'></i></a>");
