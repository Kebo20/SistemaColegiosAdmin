...
public class ClaseJpaController implements Serializable {
    ...
    public List<Clase> listarxseccion(int seccion,int año) {

        List<Clase> lista = null;
        try {
            em.getTransaction().begin();
            Query q = em.createQuery("select c from Clase c where c.idseccion.id = :seccion and c.idano.id = :año", Clase.class);
