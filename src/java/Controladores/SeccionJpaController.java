...
public class SeccionJpaController implements Serializable {
    ...
    public SeccionJpaController() {
     ...
     public List<Seccion> Listarxgrado(int idgrado) {

        List<Seccion> lista = null;
        try {
            em.getTransaction().begin();
            Query q = em.createQuery("select a from Seccion a where a.idgrado.id=:grado ", Seccion.class);
