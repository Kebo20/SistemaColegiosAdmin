...
<html lang="en">
    ...
    <body class="fix-header fix-sidebar card-no-border logo-center">
                    ...
                    <div class="row page-titles">
                        <div class="col-md-5 col-8 align-self-center">
                            ...
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">Gestionar Matrícula</li>
                            </ol>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <h4 class="card-title">Lista de Matrículas</h4>

                                    <button  data-toggle="modal" data-target="#ModalRM" class="btn btn-dropbox" >Añadir</button>

                                    <h6 class="card-subtitle"></h6>
                                    <div class="table-responsive m-t-40">
                                        <table id="tableMatricula" class="display nowrap table table-hover table-striped table-bordered text-center text-dark" cellspacing="0" width="100%">
                                            <thead>
                                                <tr>
                                                    <th>Fecha</th>
                                                    <th>Descripción</th>
                                                    <th>Alumno</th>
                                                    <th>Sección - Grado - Nivel</th>
                                                    <th>Año</th>
                                                    <th>Clases</th>
                                                    <th></th>
                                                </tr>
        ...
        <script >
            $(document).on('submit', '#formRmatricula', function () {
                $.ajax({
                    type: 'POST',
                    url: '../RegistrarMatricula',
                    data: {
                        fecha: $("#fechaRM").val(),
                        descripcion: $("#descripcionRM").val(),
                        idalumno: $("#idalumnoRM").val(),
                        idseccion: $("#idseccionRM").val(),
                        idano: $("#idanoRM").val()
                    },
                    success: function (result) {
                        swal("Correcto", result, "success");
                        document.forms[0].reset();
                        $('#tableMatricula').DataTable().ajax.reload();
                        $('#ModalRM').modal("hide");
                    }
                });
                return false;
            ...
            $(document).on('submit', '#formEmatricula', function () {
                $.ajax({
                    type: 'POST',
                    url: '../EditarMatricula',
                    data: {
                        id: $("#idEM").val(),
                        fecha: $("#fechaEM").val(),
                        descripcion: $("#descripcionEM").val(),
                        idalumno: $("#idalumnoEM").val(),
                        idseccion: $("#idseccionEM").val(),
                        idano: $("#idanoEM").val()
                    },
                    success: function (result) {
                        $('#ModalEM').modal("hide");
            ...
            $(document).on('submit', '#formELmatricula', function () {
                $.ajax({
                    type: 'POST',
            ...
            function editarMatricula(id, fecha, descripcion, idalumno, idseccion, idano) {

                $("#idEM").val(id);
                $("#fechaEM").val(fecha);
                $("#descripcionEM").val(descripcion);
                $("#idalumnoEM").val(idalumno);
                $("#idalumnoEM").change();
                $("#idseccionEM").val(idseccion);
                $("#idseccionEM").change();
                $("#idanoEM").val(idano);
                $("#idanoEM").change();
                $("#ModalEM").modal("show");
        ...
        <div id="ModalRM" class="modal fade" name='modal' role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form id="formRmatricula">
                        <div class="modal-body">

                            <div class="form-group">
                                <label for="recipient-name" class="control-label">Fecha</label>
                                <input type="date" class="form-control" id="fechaRM" required />
                            </div>

                            <div class="form-group">
                                <label for="recipient-name" class="control-label">Descripción</label>
                                <input type="text" class="form-control" id="descripcionRM" required />
                            </div>

                            <div class="form-group">
                                <label for="recipient-name" class="control-label">Alumno</label>
                                <select id="idalumnoRM" class="select2" style="width: 100%"  required="">
                                    <%
                                        AlumnoJpaController a = new AlumnoJpaController();
                                    %>
                                    <option value="">Selecciona un alumno</option>
                                    <% for (Alumno alumno : a.findAlumnoEntities()) {%>
                                    <option value="<%=alumno.getId()%>"><%=alumno.getNombre()%> <%=alumno.getApPaterno()%> <%=alumno.getApMaterno()%></option>
                                    <%}%>
                                </select> 
                            </div>

                            <div class="form-group">
                                <label for="recipient-name" class="control-label">Sección - Grado - Nivel</label>
                                <select id="idseccionRM" class="select2" style="width: 100%"  required="">
                                    <%
                                        SeccionJpaController s = new SeccionJpaController();
                                    %>
                                    <option value="">Selecciona una sección</option>
                                    <% for (Seccion seccion : s.findSeccionEntities()) {%>
                            ...
                            <div class="form-group">
                                ...
                                <select id="idanoRM" class="select2" style="width: 100%"  required="">
                                    <%
                                        AnoJpaController an = new AnoJpaController();
                                    %>
                                    <option value="">Selecciona un año</option>
                                    <% for (Ano ano : an.findAnoEntities()) {%>
        ...
        <!-- sample modal content -->
        <div id="ModalELM" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    ...
                    <form id="formELmatricula">
                        <div class="modal-body">

                            <div class="form-group">
                                <label for="recipient-name" class="control-label text-dark">¿Está seguro de eliminar esta matricula?</label>

                                <input  class="form-control" id="idELM" type="hidden" />
                            </div>
